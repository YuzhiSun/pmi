...
from PMI_tidy.code.util_snap import make_train_data
...
class ProteinMetaDataset(data.Dataset):
    def __init__(self, x,y,predict=False):
...
class MGAT_GCN_DNN(nn.Module):
    def __init__(self,drop_rate, vector_size):
        ...
        self.flatten1 = nn.Flatten()
        self.flatten2 = nn.Flatten()
        self.linear_kw = nn.Linear(684,8)
...
class GCN_DNN(nn.Module):
    def __init__(self,drop_rate, vector_size):
        ...
        self.linear_head = nn.Sequential(nn.Linear(288,8), nn.ReLU())
        self.layer1 = nn.Sequential(nn.Linear(3496, 1024), nn.BatchNorm1d(1024), nn.Dropout(drop_rate))
        self.layer2 = nn.Sequential(nn.Linear(1024, 300), nn.BatchNorm1d(300), nn.Dropout(drop_rate))
        self.out = nn.Sequential(nn.Linear(300, 2))
    def forward(self, x):
        kw = x[:,2756:-40]
...
def cross_validation(feature_matrix, label_matrix, model_name, event_num, seed,
                     ...
                     drop_rate, vector_size_input, lr, model_res_fname):
    y_true = np.array([])
    y_pred = np.array([])
    ...
    for k in range(CV):
        ...
        os.makedirs(f'../data/snap/model/{model_res_fname}', exist_ok=True)
        model_save_path = f'../data/snap/model/{model_res_fname}/{str(k)}.pt'
        ...
        os.makedirs(f'../data/snap/predictRes/{model_res_fname}', exist_ok=True)
        pred_score_path = f'../data/snap/predictRes/{model_res_fname}/{str(k)}.txt'
...
if __name__ == '__main__':
    ...
    data_for_train = make_train_data(interaction_file, standard=True, pca=False, sample=False)
    ...
    cross_validation(feature_matrix=feature_matrix, label_matrix=label_matrix,
                     ...
                     lr=lr, model_res_fname=model_res_fname)