...
def make_graph(fea_dims=20,device=None,use_rnd_feas=False,kind='test'):
    """蛋白质索引"""
    protein_node_DF = pd.read_csv(f'../data/stitch/{kind}/net/protein_node_id.csv')
    protein_node_DF.rename(columns={'index':'protein_index','Entry':'protein_id'}, inplace=True)
    protein_nums = protein_node_DF.shape[0]

    """代谢物索引"""
    meta_node_DF = pd.read_csv(f'../data/stitch/{kind}/net/meta_node_id.csv')
    meta_node_DF.rename(columns={'index':'meta_index','cid':'meta_id'}, inplace=True)
    meta_nums = meta_node_DF.shape[0]

    """蛋白质和代谢物边"""
    edge_pm_DF = pd.read_csv(f'../data/stitch/{kind}/origin_info/{kind}_interaction_with_neg.csv')
    ...
    """蛋白质间作用边"""
    edge_pp_DF = pd.read_csv(f'../data/stitch/{kind}/net/protein_interaction_net.csv')
...
def parse_node_embeddings(node_embeddings,kind):
    ...
    meta_node_id_map = pd.read_csv(f'../data/stitch/{kind}/net/meta_node_id.csv')
    protein_node_id_map = pd.read_csv(f'../data/stitch/{kind}/net/protein_node_id.csv')
    ...
    meta_embedding_df.to_csv(f'../data/stitch/{kind}/features/meta_gcn_feas.csv', index=False)
    protein_embedding_df.to_csv(f'../data/stitch/{kind}/features/protein_gcn_feas.csv', index=False)
...
if __name__ == '__main__':
    ...
    hetero_graph = make_graph(fea_dims,device,use_rnd_feas,kind)
    ...
    #保存模型
    path = f'../data/stitch/{kind}/model/dgl_model_homo.pt'
