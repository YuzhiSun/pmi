...
class MGAT_GCN_DNN(nn.Module):
    ...
    def forward(self, x):
        ...
        all = F.log_softmax(self.out(all), dim=1)
        return all
...
def train(dataloader, model, loss_fn, optimizer,device, print_res):
    ...
    if print_res == True :
        print(f'train loss: {loss:.7f} , {(end - start):.2f} seconds')
...
def test(dataloader, model, loss_fn, device, print_res):
    ...
    with torch.no_grad():
        for X, y in dataloader:
            ...
            correct.append((pred.argmax(1) == y).type(torch.float).mean().item())
            prob = pred.cpu().numpy()
...
def cross_validation(feature_matrix, label_matrix, model_name, event_num, seed,
    ...
    for k in range(CV):
        ...
        test_loader = data.DataLoader(test_dataset, batch_size=1, num_workers=4)
        ...
        for t in range(epochs):
            ...
            if print_res == True:
                print(f'------------------------\n epoch {t + 1} lr: {lr_k}  ')
            optimizer = torch.optim.Adam(model.parameters(), lr=lr_k)
            ...
            if t == 0:
                ...
                patient_k -= 1
            if patient_k == 0 and lr_k < 0.0001:
                break
            elif patient_k == 0 and lr_k >= 0.0001:
                lr_k = lr_k * 0.1
                patient_k = early_stop
        model = model_name(drop_rate, vector_size)
...
if __name__ == '__main__':
    """结果文件名称"""
    model_res_fname = 'MGAT_GCN_DNN_homo'
    """选择使用的模型"""
