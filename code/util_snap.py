...
def make_id_for_protein_and_meta():
    ...
    meta_id[['index', 'cid']].to_csv('../data/snap/net/meta_node_id.csv', index=False)
    protein_id[['index', 'Entry']].to_csv('../data/snap/net/protein_node_id.csv', index=False)
    print()
...
def make_network():
    ...
    id_mapping = pd.read_csv(r'../data/snap/origin_info/protein_id_mapping.csv')
    ...
    protein_net.to_csv('../data/snap/net/protein_interaction_net.csv',index=False)
...
def make_train_data(interaction_csv_path='../data/snap/origin_info/snap_interaction_with_neg.csv',
    ...
    res.head(10).to_csv('../data/snap/train/train_valid_demo.csv', index=False)
...
def process_snap():
    ...
    snap_meta = pd.read_csv('../data/snap/origin_info/ChG-InterDecagon_targets.csv', skiprows=1,names=['cid','gene'])
    target_meta = set(snap_meta['cid'].apply(lambda x: int(get_num(x)[0])).tolist())
    target_protein = set(snap_meta['gene'].tolist())
    # pd.DataFrame(list(target_protein)).to_csv('../data/snap/origin_info/snap_protein_id.csv',index=False,header=None)
    # pd.DataFrame(list(target_meta)).to_csv('../data/snap/origin_info/snap_cid.csv',index=False)
    # 通过uniprot网站在线转换id
    snap_info = snap_meta
    snap_info['meta_id'] = snap_meta['cid'].apply(lambda x: int(get_num(x)[0]))
    uniprot = pd.read_csv('../data/snap/origin_info/snap_protein_id_uniportkb.tsv', sep='\t')
    snap_info = pd.merge(snap_info, uniprot, left_on='gene', right_on='From', how='left')
    snap_info.drop(columns=['From'],inplace=True)
    snap_info.to_csv('../data/snap/origin_info/snap_interaction.csv', index=False)
    print()
def interaction_filter():
    all_interaction = pd.read_csv('../data/snap/origin_info/snap_interaction.csv')
    all_interaction = all_interaction[['meta_id','Entry']]
    all_interaction.rename(columns={'meta_id':'cid'}, inplace=True)
    protein_feas_id = pd.read_csv('../data/snap/features/snap_protein_kw_feas.csv')
    Entrys = set(protein_feas_id['id'].tolist())
    meta_feas_id = pd.read_csv('../data/snap/features/meta_feas.csv')
    Cids = set(meta_feas_id['KEGG'].tolist())
    ...
    target_interaction.to_csv('../data/snap/origin_info/snap_interaction_filtered.csv', index=False)
"""构建负集"""
def make_neg():
    import random
    interaction = pd.read_csv('../data/snap/origin_info/snap_interaction_filtered.csv')
    ...
    all_sample_df.to_csv('../data/snap/origin_info/snap_interaction_with_neg.csv', index=False)
...
if __name__ == '__main__':
    ...
    # make_train_data()
    """12. 使用k_fold训练模型"""
    """make data for case study 1"""
    make_train_data('/home/yuzhi/project2023/PMI_tidy/data/snap/case_study/case_to_predict.csv',standard=True,pca=False,sample=False)
