...
def make_features_of_keywords():
    originDF = pd.read_csv('../data/stitch/Homo/features/proteinFeas.csv')
    target_path = '../data/stitch/Homo/features/protein_kw_feas.csv'
    keyList = []
...
def make_id_for_protein_and_meta():
    interaction = pd.read_csv(r'../data/stitch/Homo/origin_info/snap_interaction_filtered.csv')
    ...
    meta_id[['index', 'cid']].to_csv('../data/stitch/Homo/net/meta_node_id.csv', index=False)
    protein_id[['index', 'Entry']].to_csv('../data/stitch/Homo/net/protein_node_id.csv', index=False)
    print()
...
def make_network():
    ...
    id_mapping = pd.read_csv(r'../data/stitch/Homo/origin_info/protein_id_mapping.tsv', sep='\t')
    ...
    protein_net.to_csv('../data/stitch/Homo/net/protein_interaction_net.csv',index=False)
...
def pca_deal(df, dim_percent):
    ...
    return feas
def make_train_data(interaction_csv_path='../data/stitch/Homo/origin_info/Homo_interaction_with_neg.csv',
                                   standard=False, pca = False, sample=False):
    ...
    kwDF = pd.read_csv('../data/stitch/Homo/features/protein_kw_feas.csv')
    metaDF = pd.read_csv('../data/stitch/Homo/features/meta_cid_feas.csv')
    metaGcnDF = pd.read_csv('../data/stitch/Homo/features/meta_gcn_feas.csv')
    proteinGcnDF = pd.read_csv('../data/stitch/Homo/features/protein_gcn_feas.csv')
    ...
    res.head(10).to_csv('../data/stitch/Homo/train/train_valid_demo.csv', index=False)
...
def process_stitch():
    ...
    origin_interaction = pd.read_csv('../data/stitch/Homo/origin_info/9606.protein_chemical.links.v5.0.tsv', sep='\t')
    ...
    # pd.DataFrame(list(target_protein)).to_csv('../data/stitch/Homo/origin_info/stitch_homo_protein_id.csv',index=False,header=None)
    # 通过uniprot网站在线转换id
    protein_id = pd.read_csv('../data/stitch/Homo/origin_info/protein_id_mapping.tsv', sep='\t')
    ...
    # pd.DataFrame(list(target_protein)).to_csv('../data/stitch/Homo/origin_info/protein_entry.csv',index=False,header=None)

    target_interaction.to_csv('../data/stitch/Homo/origin_info/homo_interaction.csv', index=False)
    print()
def interaction_filter():
    all_interaction = pd.read_csv('../data/stitch/Homo/origin_info/homo_interaction.csv')
    all_interaction = all_interaction[['cid','Entry']]
    protein_feas_id = pd.read_csv('../data/stitch/Homo/features/protein_kw_feas.csv')
    Entrys = set(protein_feas_id['id'].tolist())
    meta_feas_id = pd.read_csv('../data/stitch/Homo/features/metaFea.csv')
    meta_id_mapping = pd.read_csv('../data/stitch/Homo/origin_info/target_metabolite.csv', usecols=[0,1], names=['KEGG','cid'])
    ...
    meta_features.to_csv('../data/stitch/Homo/features/meta_cid_feas.csv', index=False)
    ...
    # target_interaction.to_csv('../data/stitch/Homo/origin_info/homo_interaction_filtered.csv', index=False)
"""构建负集"""
def make_neg():
    import random
    interaction = pd.read_csv('../data/stitch/Homo/origin_info/homo_interaction_filtered.csv')
    ...
    all_sample_df.to_csv('../data/stitch/Homo/origin_info/Homo_interaction_with_neg.csv', index=False)
...
if __name__ == '__main__':
    ...
    """获取蛋白质keywords信息"""
    # path = '../data/stitch/Homo/origin_info/protein_entry.csv'
    ...
    # keywordsDF.to_csv('../data/stitch/Homo/features/proteinFeas.csv', index=False)
