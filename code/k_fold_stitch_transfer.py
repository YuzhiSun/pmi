...
class TF(nn.Module):
    def __init__(self,  drop, in_features):
        super().__init__()
        self.Block1 = Mlp(in_features=int(in_features), hidden_features=64, act_layer=nn.GELU, drop=drop, pred=False)
        ...
        # self.Block1_1 = Mlp(in_features=in_features, hidden_features=64, act_layer=nn.GELU, drop=drop, pred=False)
        # self.Block1_1 = Mlp(in_features=in_features, hidden_features=64, act_layer=nn.GELU, drop=drop, pred=False)
        self.Block2 = Mlp(in_features=int(in_features), hidden_features=64, act_layer=nn.GELU, drop=drop, pred=True)
...
class MGAT_GCN_DNN(nn.Module):
    def __init__(self,drop_rate, vector_size):
        ...
        self.flatten1 = nn.Flatten()
        self.flatten2 = nn.Flatten()
        self.linear_kw = nn.Linear(633,8)
        ...
        self.layer1 = nn.Sequential(nn.Linear(vector_size + 8, 1024), nn.BatchNorm1d(1024), nn.Dropout(drop_rate))
...
class GCN_DNN(nn.Module):
    def __init__(self,drop_rate, vector_size):
        ...
        self.linear_head = nn.Sequential(nn.Linear(288,8), nn.ReLU())
        self.layer1 = nn.Sequential(nn.Linear(3445, 1024), nn.BatchNorm1d(1024), nn.Dropout(drop_rate))
        self.layer2 = nn.Sequential(nn.Linear(1024, 300), nn.BatchNorm1d(300), nn.Dropout(drop_rate))
        self.out = nn.Sequential(nn.Linear(300, 2))
    def forward(self, x):
        kw = x[:,2756:-40]
...
def cross_validation(feature_matrix, label_matrix, model_name, event_num, seed,
                     ...
                     drop_rate, vector_size_input, lr, model_res_fname, kind, src_model_pth):
    y_true = np.array([])
    y_pred = np.array([])
    ...
    for k in range(CV):
        ...
        src_dict = {k:v for k, v in src_dict.items() if (k.find('head')!=-1) & (k.find('weight')!=-1)}
        ...
        os.makedirs(f'../data/stitch/{kind}/model/{model_res_fname}', exist_ok=True)
        model_save_path = f'../data/stitch/{kind}/model/{model_res_fname}/{str(k)}.pt'
        ...
        os.makedirs(f'../data/stitch/{kind}/predictRes/{model_res_fname}', exist_ok=True)
        pred_score_path = f'../data/stitch/{kind}/predictRes/{model_res_fname}/{str(k)}.txt'
...
if __name__ == '__main__':
    """结果文件名称"""
    model_res_fname = 'test'
    src_model_pth = '../data/snap/model/MGAT_GCN_DNN/0.pt'
    kind = 'Homo'
    """选择使用的模型"""
    ...
    "特征文件"
    data_for_train = make_train_data(kind, standard=True, pca=False, sample=False)
    ...
    cross_validation(feature_matrix=feature_matrix, label_matrix=label_matrix,
                     ...
                     lr=lr, model_res_fname=model_res_fname,kind=kind, src_model_pth=src_model_pth)