...
def make_graph(fea_dims=20,device=None):
    """蛋白质索引"""
    protein_node_DF = pd.read_csv('../data/self/net/protein_node_id.csv')
    protein_node_DF.rename(columns={'index':'protein_index','id':'protein_id'}, inplace=True)
    protein_nums = protein_node_DF.shape[0]

    """代谢物索引"""
    meta_node_DF = pd.read_csv('../data/self/net/meta_node_id.csv')
    meta_node_DF.rename(columns={'index':'meta_index','id':'meta_id'}, inplace=True)
    meta_nums = meta_node_DF.shape[0]

    """蛋白质和代谢物边"""
    edge_pm_DF = pd.read_csv('../data/self/origin_info/interaction_Homo.csv')
    edge_pm_DF.rename(columns={'id': 'protein_id'}, inplace=True)
    edge_pm_DF = edge_pm_DF[edge_pm_DF['interaction'] == True]
    edge_pm_DF.drop_duplicates(inplace=True)

    """蛋白质间作用边"""
    edge_pp_DF = pd.read_csv(r'../data/self/net/protein_interaction_net.csv')
    ...
    p_e_m_df = pd.merge(p_e_m_df, meta_node_DF, how='left', left_on='KEGG', right_on='meta_id')
...
def parse_node_embeddings(node_embeddings):
    ...
    meta_node_id_map = pd.read_csv('../data/self/net/meta_node_id.csv')
    protein_node_id_map = pd.read_csv('../data/self/net/protein_node_id.csv')
    ...
    meta_embedding_df.to_csv('../data/self/features/meta_gcn_embedding_feas.csv', index=False)
    protein_embedding_df.to_csv('../data/self/features/protein_gcn_emgbedding_feas.csv', index=False)
...
if __name__ == '__main__':
    ...
    #保存模型
    path = '../data/self/model/dgl_model_rnd.pt'
